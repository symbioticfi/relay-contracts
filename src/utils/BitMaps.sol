...
library BitMaps {
    ...
    function get(BitMap storage bitmap, uint256 index) internal view returns (bool) {
        ...
        return bitmap._data[bucket].latest() & mask != 0;
    }

    function getAt(BitMap storage bitmap, uint256 index, uint48 timestamp) internal view returns (bool) {
        uint256 bucket = index >> 8;
        uint256 mask = 1 << (index & 0xff);
    ...
    function getBucket(BitMap storage bitmap, uint256 bucket) internal view returns (uint256) {
        return bitmap._data[bucket].latest();
    }

    function getBucketAt(BitMap storage bitmap, uint256 bucket, uint48 timestamp) internal view returns (uint256) {
