...
library KeyRegistry32 {
    ...
    struct Registry {
        ...
        mapping(bytes32 => uint256) keyData;
    }

    function getOperatorByKey(Registry storage self, bytes32 key) internal view returns (address) {
        return self.keyData[key].getOperator();
    }

    function getKeyPosition(Registry storage self, bytes32 key) internal view returns (uint256) {
        return self.keyData[key].getPosition();
    }

    function getOperatorKeys(Registry storage self, address operator) internal view returns (bytes32[] memory) {
    ...
    function getEnabledOperatorKeys(Registry storage self, address operator) internal view returns (bytes32[] memory) {
        return Subsets.getEnabledBytes32Subset(self.keys[operator], self.keysStatus[operator], Time.timestamp());
    }
    ...
    {
        return Subsets.getEnabledBytes32Subset(self.keys[operator], self.keysStatus[operator], timestamp);
    }

    function registerOperatorKey(Registry storage self, address operator, bytes32 key) internal {
