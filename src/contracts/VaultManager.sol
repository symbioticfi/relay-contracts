...
contract VaultManager is NetworkConfig {
    ...
    function addSharedVault(address vault, uint208 weight) public onlyRole(SHARED_VAULT_ADD_ROLE) {
        VaultManagerLogic.addSharedVault(
            _getVaultManagerStorage(), _getOperatorManagerStorage(), _getNetworkConfigStorage(), VAULT_FACTORY, vault, weight
        );
    }
    ...
    function addOperatorVault(
        ...
        VaultManagerLogic.addOperatorVault(
            _getVaultManagerStorage(), _getOperatorManagerStorage(), _getNetworkConfigStorage(), VAULT_FACTORY, operator, vault, weight
        );
    }
    ...
    function requestSlash(address operator, address vault, uint256 amount, bytes memory hints) public onlyRole(SLASH_REQUEST_ROLE) {
