...
import {KeyManager} from "../../base/abstracts/KeyManager.sol";
...
abstract contract MultiKeyManager is KeyManager, EIP712Upgradeable {
    ...
    // keccak256(abi.encode(uint256(keccak256("symbiotic.storage.MultiKeyManager")) - 1)) & ~bytes32(uint256(0xff))
    ...
    function getKey(address operator, uint8 tag) public view virtual override returns (bytes memory) {
        KeyType type_ = KeyType(tag.getType());
        if (type_ == KeyType.BLS_BN254) {
    ...
    function _registerKeys(address operator, KeyWithSignature[] memory keysWithSignatures) public {
        ...
        _getKeyManagerConfig()._operators.add(Time.timestamp(), operator);
    }
