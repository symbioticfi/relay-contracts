...
library VaultManagerLogic {
    ...
    function requestSlash(
        ...
        if (slasherType == uint64(SlasherType.INSTANT)) {
            response = abi.encode(ISlasher(slasher).slash(NetworkConfigLogic.getSubnetwork(networkConfigStorage), operator, amount, captureTimestamp, hints));
        } else if (slasherType == uint64(SlasherType.VETO)) {
            response = abi.encode(IVetoSlasher(slasher).requestSlash(NetworkConfigLogic.getSubnetwork(networkConfigStorage), operator, amount, captureTimestamp, hints));
        }
    }
    ...
    function _validateVault(
        ...
        if (self._allOperatorVaults.isIncluded(currentEpoch + 1, vault) || self._sharedVaults.isIncluded(currentEpoch + 1, vault)) {
