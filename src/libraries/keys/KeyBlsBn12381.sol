...
/**
 * @title KeyBlsBn12381
 * @notice Library for interacting with BLS public keys on the BN12381 curve, including validation, serialization, and compression.
 * @dev The keys are represented as G1 points on the BN12381 curve.
 */
library KeyBlsBn12381 {
    using KeyBlsBn12381 for BN12381.G1Point;
    using KeyBlsBn12381 for KEY_BLS_BN12381;
    using BN12381 for BN12381.G1Point;
    using BN12381 for uint256[2];
    ...
    function wrap(BN12381.G1Point memory keyRaw) internal view returns (KEY_BLS_BN12381 memory key) {
        ...
        if (
            uint256(keyRaw.x_a) > BN12381.P_A
                || (uint256(keyRaw.x_a) == BN12381.P_A && uint256(keyRaw.x_b) > BN12381.P_B)
                || uint256(keyRaw.y_a) > BN12381.P_A
                || (uint256(keyRaw.y_a) == BN12381.P_A && uint256(keyRaw.y_b) > BN12381.P_B)
        ) {
            revert KeyBlsBn12381_InvalidKey();
        }
        if (!BN12381.isOnCurve(keyRaw)) {
            revert KeyBlsBn12381_InvalidKey();
        }
        if (!BN12381.isInSubgroup(keyRaw)) {
            revert KeyBlsBn12381_InvalidKey();
        }
        key = KEY_BLS_BN12381(keyRaw);
    }
    ...
    function serialize(KEY_BLS_BN12381 memory key) internal view returns (bytes memory keySerialized) {
        ...
        (uint256 derivedY_a, uint256 derivedY_b) = BN12381.findYFromX(uint256(key.value.x_a), uint256(key.value.x_b));
    ...
    function deserialize(bytes memory keySerialized) internal view returns (KEY_BLS_BN12381 memory key) {
        ...
        if (compressedKey[0] == 0 && compressedKey[1] == 0) {
            return KEY_BLS_BN12381(BN12381.G1Point({x_a: 0, x_b: 0, y_a: 0, y_b: 0}));
        }
        uint256 x_a = compressedKey[0] >> 1;
        (uint256 derivedY_a, uint256 derivedY_b) = BN12381.findYFromX(uint256(x_a), uint256(compressedKey[1]));
        key.value = BN12381.G1Point({
            x_a: bytes32(x_a), x_b: bytes32(compressedKey[1]), y_a: bytes32(derivedY_a), y_b: bytes32(derivedY_b)
        });
        if (compressedKey[0] & 1 > 0) {
            key.value = BN12381.negate(key.value);
        }
    }
    ...
    function fromBytes(bytes memory keyBytes) internal view returns (KEY_BLS_BN12381 memory key) {
        key.value = abi.decode(keyBytes, (BN12381.G1Point));
        if (keccak256(key.unwrap().wrap().toBytes()) != keccak256(keyBytes)) {
            revert KeyBlsBn12381_InvalidBytes();
        }
    }
    ...
    function zeroKey() internal view returns (KEY_BLS_BN12381 memory key) {
        key = KEY_BLS_BN12381(BN12381.G1Point({x_a: 0, x_b: 0, y_a: 0, y_b: 0}));
    }
